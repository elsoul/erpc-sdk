...
const ASSET_KINDS = new Set(["native", "issued", "wrapped", "bridged"]);
...
function renderTypeScript(catalog) {
  ...
  `export const TOKEN_ALIASES: readonly TokenAlias[] = deepFreeze(${tsValue(orderedAliases(catalog))}) as readonly TokenAlias[];`,
...
const RUST_ENUMS = Object.freeze({
  representationKind: { native: "Native", issued: "Issued", wrapped: "Wrapped", bridged: "Bridged" },
...
function renderRust(catalog) {
  ...
  for (const alias of orderedAliases(catalog)) {
    lines.push(
      ...
      `        deployment_id: ${rustString(alias.deploymentId)},`,
      "    },",
    );
  }
  lines.push("];", "", "pub mod tokens {");
  for (const [group, entries] of Object.entries(groups)) {
    lines.push(`    pub mod ${group} {`);
    for (const entry of entries) lines.push(`        pub const ${entry.property}: &str = ${rustString(entry.deploymentId)};`);
    lines.push("    }");
  }
  lines.push("}", "");
  return lines.join("\n");
...
function renderPython(catalog) {
  ...
  for (const alias of orderedAliases(catalog)) lines.push(`    TokenAlias(${pythonValue(alias.namespace)}, ${pythonValue(alias.name)}, ${pythonValue(alias.deploymentId)}),`);
  lines.push(")", "");
  for (const [group, entries] of Object.entries(groups)) {
    const className = `_${identifier(group, "python").split("_").map((part) => part ? part[0].toUpperCase() + part.slice(1) : part).join("")}Tokens`;
    lines.push(`class ${className}(NamedTuple):`);
    for (const entry of entries) lines.push(`    ${entry.property}: str`);
    lines.push("");
  }
  lines.push("class _Tokens(NamedTuple):");
  for (const group of Object.keys(groups)) lines.push(`    ${group}: _${identifier(group, "python").split("_").map((part) => part ? part[0].toUpperCase() + part.slice(1) : part).join("")}Tokens`);
  lines.push("", "tokens = _Tokens(");
  for (const [group, entries] of Object.entries(groups)) {
    const className = `_${identifier(group, "python").split("_").map((part) => part ? part[0].toUpperCase() + part.slice(1) : part).join("")}Tokens`;
    lines.push(`    ${group}=${className}(`);
    for (const entry of entries) lines.push(`        ${entry.property}=${q(entry.deploymentId)},`);
    lines.push("    ),");
  }
...
function renderGo(catalog) {
  ...
  for (const alias of orderedAliases(catalog)) lines.push(`        {Namespace: ${goString(alias.namespace)}, Name: ${goString(alias.name)}, DeploymentID: ${goString(alias.deploymentId)}},`);
  lines.push("    }", "}", "");
  for (const [group, entries] of Object.entries(groups)) {
...
function renderRuby(catalog) {
  ...
  for (const alias of orderedAliases(catalog)) lines.push(`      ${rubyValue({namespace:alias.namespace,name:alias.name,deployment_id:alias.deploymentId})}.freeze,`);
  lines.push("    ].freeze", "  end", "", "  module Tokens");
  for (const group of ["ethereum", "solana", "avalancheC"]) {
    const constant = group === "avalancheC" ? "AvalancheC" : group[0].toUpperCase() + group.slice(1);
    const entries = groups[group === "avalancheC" ? "avalanche_c" : group] ?? [];
    lines.push(`    ${constant} = ${rubyValue(Object.fromEntries(entries.map((entry) => [entry.property, entry.deploymentId])))}.freeze`);
  }
  lines.push("  end", "end", "");
  return lines.join("\n");
}
