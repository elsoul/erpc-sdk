...
function renderRust(catalog) {
  ...
  const tokenGroups = Object.entries(groups).flatMap(([group, entries]) => [
    `    pub mod ${group} {`,
    ...entries.map((entry) => `        pub const ${entry.property}: &str = ${rustString(entry.deploymentId)};`),
    "    }",
  ]);
  const lines = [
    "// Generated by registry/generate-token-catalog.mjs. Do not edit.",
...
function renderPython(catalog) {
  ...
  const aliasRows = orderedAliases(catalog).map((alias) => `    TokenAlias(${pythonValue(alias.namespace)}, ${pythonValue(alias.name)}, ${tokenAliasReference(alias, "python")}),`);
  const lines = [
    "# Generated by registry/generate-token-catalog.mjs. Do not edit.",
