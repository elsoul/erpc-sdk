...
registry for native issued EURC between Ethereum mainnet and Solana mainnet.
It is a provider integration record. It is not a CCTP burn/mint route, an
RPC-only swap capability, a settlement guarantee, or an approval to move funds.
...
There are exactly two directional IDs:

| Capability | Source | Destination | Source EURC | Destination EURC |
| --- | --- | --- | --- | --- |
...
`spl`; the mapping is explicit and limited to the provider boundary. No public
route substitution with USDC, wrapped EURC, native ETH, or native SOL is
permitted. The provider internally converts through source-chain USDC:
...
Mayan Swift contract `0x40ffe85a28dc9993541449464d7529a922142960`, Forwarder
`0x337685fdab40d39bd02028545a4ffa7d287cc3e2`, and selector `0x30dedc57`.
Solana source rows bind Swift program
`mayan34VedncxdK2XobtvWFDXQASUTBXhUVzt2kKgny` and Jupiter v6
`JUP6LkbZbjS1jKKwapdHNy74zcZ3tLUZoi5QNyVTaV4`.

## Runtime projection and generation

The registry `asOfDate` is `2026-09-16`. Generated runtime data is a plain
...
Only the two registry directions and their exact token deployment IDs pass
the local gate. The amount is snapshotted before an asynchronous request and
...
`quoteVerification: "provider-signed-not-locally-verified"`. The
`sourceSwap` object exposes the required USDC conversion, provider-selected
router, and the unchanged JSON number lexeme of `minMiddleAmount` as
`providerMinimumAmount`. That provider-internal number is not converted into
base units. Solana rows use `intermediateTokenStandard: "spl"` at this one
...
value zero, and contain a sufficiently long calldata body beginning with
`0x30dedc57`. The allowance record points EURC deployment-0011 at the Forwarder
for the exact quote input. No approval call or allowance RPC is generated.
...
is a synthetic wire fixture registry. It includes both directions, successful
quote/build/status cases, malformed and duplicate-key JSON, expiry and route
binding failures, EVM target/value/selector failures, Solana framing and
ephemeral-signer failures, authentication and rate-limit responses, redirect,
timeout, cancellation, and bounded body cases. Synthetic provider signatures,
keys, and addresses are test values; they are never authenticated captures.
The raw signed quote object retains an unknown field and the `114.5000` JSON
number lexeme so tests can detect accidental reserialization or conversion.
...
The provider, auction solvers, relayers, Wormhole Guardians, Jupiter source
swap, and Explorer indexer are external dependencies. Their deployed versions,
terms, SLA, security ownership, and data rights remain open review items.
